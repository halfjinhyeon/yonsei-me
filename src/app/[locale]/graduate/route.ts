...
// `/graduate` → 기본 탭(/graduate/requirements) 308. 탭이 경로가 되면서 이 주소 자체는
// 문서가 아니게 됐다 — 구 링크·북마크를 기본 탭으로 보낸다. 해시(#labs 등)는 브라우저가
...
export function GET(req: NextRequest, { params }: { params: { locale: string } }) {
  return legacyRedirect(req, params.locale, sectionDefaultHref('graduate'));
}
