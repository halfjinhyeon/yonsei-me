...
// `/news` → 기본 게시판 308. 탭이 경로가 되면서(/news/<탭>) 이 주소 자체는 목록이
// 아니게 됐다 — 구 링크·북마크를 기본 탭으로 보낸다. 해시(#seminars 등)는 브라우저가
// 리다이렉트를 따라가며 보존하고, 목록 페이지의 LegacyBoardHash 가 최종 탭으로 옮긴다.
...
export function GET(req: NextRequest, { params }: { params: { locale: string } }) {
  return legacyRedirect(req, params.locale, newsTabHref(DEFAULT_NEWS_TAB));
}
