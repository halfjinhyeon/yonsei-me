...
```
크롤러 repo (Desktop\크롤링, YONSEI_COOKIE 는 선택 — 아래 각주)
...
1. **(보통은 건너뛴다)** 쿠키는 선택이다 — 읽기 API 는 Cookie 헤더 없이도 같은 JSON 을
   돌려준다(2026-09-06 실측: 요약·순위·코드·과목 목록 전부). 크롤이 `JSON 대신 HTML` 로
   멈출 때만, 브라우저로 수강편람(underwood1.yonsei.ac.kr)에 접속해 무슨 화면인지 보고
   (대기열·점검·로그인 요구 = **게이트**) Network 탭 `.do` 요청의 Cookie 전체 값을
   `크롤링\.env`의 `YONSEI_COOKIE=`에 넣는다
...
- **크롤러의 `courses.json`은 마일리지 파이프라인 정본 입력**이다. `crawl-terms.mjs`는
  실행 시작 시 그것을 `courses.json.pre-checker`로 스냅샷하고 종료 시(실패 포함) 그
  스냅샷으로 복원한다 — "찾은 그대로 되돌려 놓는" 계약. 백업을 매 실행 시점에 새로 뜨는
  이유는, 고정 백업을 복원하면 다음 학기에 마일리지가 방금 크롤한 최신 courses.json을
  낡은 백업으로 덮어쓰는 사고가 나기 때문이다. 스냅샷이 곧 대상 학기의 카탈로그면 그
  학기는 크롤 없이 시드된다.
- **끊기면 이어서 돌리면 된다.** 학기별 파일 단위로 재개되므로(`--list`로 진행 확인) 같은
  명령을 다시 돌리면 이어서 진행된다. 예전엔 "쿠키 만료가 상수" 라고 적혀 있었지만,
  2026-09-06 실측으로 **읽기 API 는 쿠키 없이도 응답**한다 — 그러니 `JSON 대신 HTML` 로
  멈추는 것은 로그인 문제가 아니라 **게이트**(수강신청 기간 NetFunnel 대기열·점검·WAF)로
  보고, 브라우저로 상태를 먼저 확인한다. 쿠키는 그 게이트를 통과할 때만 넣는다.
- **서버가 엉뚱한 학기를 돌려줄 수 있다.** 래퍼가 레코드의 syy/smtDivCd를 요청 학기와
  대조해 불일치면 저장하지 않고 중단한다.
